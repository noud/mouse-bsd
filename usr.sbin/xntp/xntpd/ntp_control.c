	...
	int varid;
{
	...
	case CS_DRIFT:
		...
		break;
	case CS_COMPLIANCE:
		ctl_putufp(sys_var[CS_COMPLIANCE].text, sys_maxd[0]);
		break;
	case CS_CLOCK:
		get_systime(&tmp);
	...
	case CS_SYSTEM:
		...
		ctl_putstr(sys_var[CS_SYSTEM].text, str_system,
		    sizeof(str_system) - 1);
#else
 		ctl_putstr(sys_var[CS_SYSTEM].text, utsnamebuf.sysname,
		    strlen(utsnamebuf.sysname));
#endif /* HAVE_UNAME */
		break;
	case CS_KEYID:
